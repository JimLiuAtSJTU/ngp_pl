...
class N3DV_dataset_2(BaseDataset):
    ...
    def hexplane_sample(self):
        ...
        if self.sample_stages==1:
            ...
            rgb_train_importance = (
                self.rays_rgbs.view(self.N_cam, self.N_time, self.H * self.W, 3)[cam_i, t_index_i].view(-1, 3)
            )
            rgb_ref = (
                self.rays_rgbs.view(self.N_cam, self.N_time, self.H * self.W, 3)[cam_i, index_2].view(-1, 3)
            )
