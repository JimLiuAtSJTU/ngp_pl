...
class NeRFNetwork_2(NeRFRenderer):
    def __init__(self,
        ...
        N_min=highest_reso
        b = 1 # np.exp(np.log(highest_reso / N_min) / (L - 1))

        self.time_latent_code = tcnn.NetworkWithInputEncoding(
            n_input_dims=2, n_output_dims=13,
            encoding_config={
                "otype": "Composite",
                "nested": [
                    {
        ...
        self.encoder_time = self.time_latent_code
    ...
    def forward(self, x, d, t):
        ...
        enc_t = self.encoder_time(t.expand(1,2))  # [1, 1] --> [1, C']
        if enc_t.shape[0] == 1:
            enc_t = enc_t.repeat(x.shape[0], 1)  # [1, C'] --> [N, C']
